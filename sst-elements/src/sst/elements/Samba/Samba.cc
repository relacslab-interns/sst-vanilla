    ...
    for(uint32_t i = 0; i < core_count; ++i) {
        ...
        if(self==0)
        {
            link3 = configureLink(link_buffer3, 
                    new Event::Handler<PageTableWalker>(TLB[i]->getPTW(), &PageTableWalker::recvResp));
        }
        else
        {
        ...
        if(emulate_faults==1)
        {
            pageFaultHandler->registerPageFaultHandler(i, 
                    new PageFaultHandler::PageFaultHandlerInterface<PageTableWalker>(
                        TLB[i]->getPTW(), &PageTableWalker::recvPageFaultResp));

            TLB[i]->getPTW()->setPageFaultHandler(pageFaultHandler);

            sprintf(link_buffer, "event_bus%" PRIu32, i);
            SST::Link* self_link = configureSelfLink(link_buffer, "1ns", 
                    new Event::Handler<PageTableWalker>(TLB[i]->getPTW(), &PageTableWalker::handleEvent));

            TLB[i]->getPTW()->setEventChannel(self_link);
            TLB[i]->setPageTablePointers(&CR3, &PGD, &PUD, &PMD, &PTE, 
                    &MAPPED_PAGE_SIZE1GB, &MAPPED_PAGE_SIZE2MB, &MAPPED_PAGE_SIZE4KB, 
                    &PENDING_PAGE_FAULTS, &cr3I, &PENDING_PAGE_FAULTS_PGD, &PENDING_PAGE_FAULTS_PUD, 
                    &PENDING_PAGE_FAULTS_PMD, &PENDING_PAGE_FAULTS_PTE);//, &PENDING_SHOOTDOWN_EVENTS, &PTR, &PTR_map);
        }
...
void Samba::init(unsigned int phase) {
    ...
    /* CPU or memory may send init events to memory, pass them through */
    for (uint32_t i = 0; i < core_count; i++) {
        SST::Event * ev;
        while ((ev = cpu_to_mmu[i]->recvInitData())) {
            mmu_to_cache[i]->sendInitData(ev);

            /* PTW ports inits to let cache hierarchy be aware of it */
