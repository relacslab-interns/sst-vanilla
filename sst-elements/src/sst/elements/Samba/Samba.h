...
/*
 * Notes:
 * 1. A Samba instance assumes single VMA for all TLB hierarchy instances in itself, cause it provides one CR3
 * 2. Realistic PTW model requires Opal's page fault handler; however, up-to-date SST does not provide Opal
 * 3. Due to updates on Cache's response mechasism (sendResponseUp), Samba does not support to connect w/ cache
 *    because cache cannot recognize mmu as destination - This requires modification on Samba::init()
 */
